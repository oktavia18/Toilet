...
class M_checklist extends CI_Model
{
    ...
    {
        $this->db->from($this->table);
        $this->db->where("checklist.inspector_id", $this->session->userdata("id"));
        return $this->db->count_all_results();
    }
    ...
    function get_limit_data()
    {
        ...
        $this->db->select(
            "checklist.*, toilet.lokasi as toilet_lokasi, inspector.name as user_name"
        );
        $this->db->join("inspector", "inspector.id  = checklist.inspector_id");
